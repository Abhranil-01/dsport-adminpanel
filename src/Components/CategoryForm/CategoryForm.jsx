...
import { useCreateCategoryMutation } from "../../Services/fetchDataFromApi";
import SpinLoader from "../SpinLoader/SpinLoader";
import { toast } from "react-toastify";

function CategoryForm({ setShowForm }) {
  const [categoryName, setCategoryName] = useState("");
  const [loader, setLoader] = useState(false);
  const [imagesWithId, setImagesWithId] = useState([]); // local + backend images

  const [createCategory] = useCreateCategoryMutation();

  // ✅ Submit form with category + images
  ...
  return (
    ...
    >
      <div className="fixed left-[26%] top-[18%] bg-white shadow-lg shadow-gray-800 rounded-lg w-[60vw] h-[65vh] py-5 px-5 dark:bg-gray-900 dark:text-white dark:shadow-gray-200/100">
        <div className="flex justify-between items-center font-bold h-[5%] mb-2">
          <span className=" font-bold">Enter Category Details</span>
          <span className="cursor-pointer" onClick={() => setShowForm(false)}>
            <FontAwesomeIcon icon={faX} />
          </span>
        </div>

        <div className="relative w-full h-[90%] flex items-center justify-center border-2 rounded border-gray-800 px-7 dark:border-gray-400">
          <form
            onSubmit={handleSubmit}
            className="flex items-center justify-center gap-5 w-full h-full"
          >
            <div className="h-[45vh] w-[50%]">
              <ImageBox
                images={imagesWithId}
                editable={true}
                onAddImages={(newImgs) =>
            ...
            <div className="h-[90%] w-[50%] flex flex-col items-center justify-center">
              <InputBox
